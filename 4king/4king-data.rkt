...
(struct ENEMY (NAME AT HP) #:transparent)
(struct ITEM (NAME COST KIND POWER) #:transparent)
(struct PLAYER (NAME SKILLP HITP MAGICP EQUIP GOLD ITEMS SPECIAL WIN) #:transparent)
...
(define SA (CARD 'S 1 'BATTLE 'mes-s1 '(enemy-zakura) '() #t #t))
(define zakura (ENEMY "戦士ザクラ" 12 12))
(define S2 (CARD 'S 2 'SHOP 'mes-s2 '() '(silver-short-sord war-hammer long-sord throwing-knife) #t #f))
(define S3 (CARD 'S 3 'SHOP 'mes-s3 '() '(magic-gloves shield chain-mail) #t #f))
(define S4 (CARD 'S 4 'SHOP 'mes-s4 '() '(horse) #t #f))
(define S5 (CARD 'S 5 'SHOP 'mes-s5 '() '(numbing-medicine anesthetic medicinal-herb skill-herb power-herb luck-herb) #t #f))
(define S6 (CARD 'S 6 'SHOP 'mes-s6 #f '(acrobat performance handmagic instrument) #t #f))
...
(define SQ (CARD 'S 'Q 'BATTLE 'mes-sq '(enemy-iris) '() #t #t))
...
(define D2 (CARD 'D 2 'BATTLE 'mes-d2 '(orc1 orc2) '((gold . 6)) #t #t)) ;goldはCARDのITEMに
...
(define D6 (CARD 'D 6 'PICKPOCKET'mes-d6 '(pickpoketer) '((gold . 18)) #t #t))
(define pickpocketer (ENEMY "スリ" 7 7))
(define D7 (CARD 'D 7 'BATTLE 'mes-d7 '(mouse1 mouse2 mouse3 mouse4) '((gold . 21)) #t #t)) ;悪臭ステータス追加
...
(define D9 (CARD 'D 9 'BATTLE 'mes-d8 '(monster-fish) '((gold . 27)) #t #t))
(define D10 (CARD 'D 10 'BATTLE 'mes-d9 '(oltergeist) '((gold . 30)) #t #t))
(define DQ (CARD 'D 'Q 'BATTLE 'mes-dq '(assassin) '(poison-dagger) #t #t))
(define poison-dagger (ITEM "毒の短剣" #f 'poison-weapon '(0 0)))
(define DK (CARD 'D 'K 'ARENA 'mes-dk '(porcupine wolf panther dragger cyclops hole-devil) '() #t #t))
